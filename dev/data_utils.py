...
def get_dataset(name, path="./data", split_type="public", reorder_label=False):
    ...
    if name in ["Cora", "CiteSeer", "PubMed"]:
        ...
        dataset = Amazon(path + "/amazon", name, transform=T.NormalizeFeatures())
        ...
        dataset = CoraFull(path + "/CoraFull", transform=T.NormalizeFeatures())
    elif name in ["Reddit"]:
...
def get_train_val_test_split(data, n_train_num_per_class, test_ratio=0.5):
    ...
    train_mask = index_to_mask(torch.concat(train_index), size=n_nodes)
    val_mask = index_to_mask(torch.concat(val_index), size=n_nodes)
    test_mask = index_to_mask(torch.concat(test_index), size=n_nodes)
